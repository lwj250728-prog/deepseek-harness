...
def _leak_scan() -> list:
    ...
    而且真的发生过(dsh-probe-binding.py 带着 T230 的变异体, 锚点漂移检查被静默关掉, 闸门因白名单过宽而看不见)。
    抽成函数是因为第一版把它内联在**写回之后** ⇒ `--write-arms` 直接 `UnboundLocalError: leaks`(把测量写回路径弄崩)。"""
    gate = os.path.join(os.path.expanduser('~/dsh-fork'), 'dsh-mutant-gate.py')
    leaks = []
    if os.path.exists(gate):
        try:
            g = subprocess.run([sys.executable, gate, '--check', '--json'], capture_output=True, text=True, timeout=600)
            for line in reversed([x for x in (g.stdout or '').strip().splitlines() if x.strip().startswith('{')]):
                try:
                    leaks = (json.loads(line) or {}).get('leaks') or []
                    break
                except Exception:
                    continue
...
def main() -> int:
    ...
    leaks = _leak_scan()   # **先扫残留, 再写回**(第一版放在写回之后 ⇒ --write-arms 直接 UnboundLocalError)
    if leaks:
