...
- The chain is a DERIVED view over tagged atoms: atoms accumulate online, chains consolidate offline (at goal completion / agent idle — the systems-consolidation analogue). A chain without tags never forms, and channels that do not tag (e.g. plain subagent delegation outside the orchestrator) still produce orphan scenes — the abstraction does not fix untagged channels, it names the gap (exp_73's mechanism). Patterns inherit that dependence: they aggregate only the chains that actually formed.
