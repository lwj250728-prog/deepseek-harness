...
- Fifteen model tools (was fourteen); a sixth persisted table (`chain_patterns.json`, plus `chains.json`); the injection citation loop now measures chain-level and pattern-level utility too.
- The chain is a DERIVED view over tagged atoms: atoms accumulate online, chains consolidate offline (at goal completion / agent idle — the systems-consolidation analogue). A chain without tags never forms. The remember tool path now carries the tag (`remember_experience` accepts `chain_id`, the goal trace id), so a caller executing a goal can tag each member as it is remembered — closing the exp_73 gap that untagged channels produced orphan scenes. For atoms that were NEVER tagged, `explore_chain` recovers the goal structure from text instead: upstream/downstream neighbors by outcome→situation承接 are suggested (never written), so a scattered store can still be assembled into chains by a caller who tags and consolidates. Channels that still do not tag keep producing orphan scenes; the abstraction does not fix untagged channels, it names the gap and offers the recovery tool. Patterns inherit that dependence: they aggregate only the chains that actually formed.
...
- Self-reflexive turns are annotated at the accumulation gate: a tool call whose arguments carry a kill/restart signature (Stop-Process, taskkill, net stop) marks the turn `selfReflexive`, and the gate input is annotated "[推测性行动]" — the causal chain after killing one's own host is unobservable from the session ledger, so the reconstructed action must not be asserted as fact without external witnessing (the exp_155 lesson: the LLM hallucinated "restarted the service" as the agent's own action when an external actor had done it). This is the pipeline's version of the human lesson from state-dependent memory: trust the external record (state file, logs) over the broken internal narrative.
