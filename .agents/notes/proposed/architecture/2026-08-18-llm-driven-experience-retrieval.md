...
The pipeline's stated design intent is an experience network retrieved with model capacity (the design's `all-MiniLM-L6-v2` / `text-embedding-3-small` retrieval axis), not a flat list matched by hashed keywords. The current implementation treats the deterministic floor as the architecture: the [symptom-first recall work](../feature/2026-08-18-symptom-first-sar-extraction-and-recall.md) improved the floor but did not move toward the ceiling. The bug-scale, fine-grained handling that motivated those rounds is the *lower bound demonstration*, not the design target.
