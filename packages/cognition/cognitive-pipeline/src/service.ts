...
import { strategyDueForReview } from './review-schedule.ts'
import type {
  AcceptanceCheck,
  AcceptanceProposal,
  CalibrationBucket,
...
export class CognitivePipelineService extends Service {
   ...
   */
  /** How many due strategies one review pass re-checks at most. */
  private static readonly REVIEW_PASS_CAP = 3

  /** Minimum gap between two re-checks of the same strategy (rework-flagged
   * strategies are always "due" by the rule; the cooldown keeps an idle loop
   * from hammering a broken anchor every tick). */
  private static readonly REVIEW_RECHECK_COOLDOWN_MS = 10 * 60 * 1000

  /** Run one idle drift re-verification pass over solidified strategies: the
   * due ones (activation rule; rework-flagged always due) have their
   * verification anchor re-checked — a real command run only when command
   * execution is enabled AND the anchor reads as an ASCII command line;
   * otherwise the re-check is unverified (clock refresh only). Each re-check
   * folds its verdict into the validity ledger and refreshes the activation
   * clock. Runs on the offline-consolidation idle cadence.
   * @returns the pass summary.
   */
  async runStrategyReviewPass(): Promise<{
    reviewed: number
    anchorHeld: number
    anchorFailed: number
    unverified: number
    ...
    const now = Date.now()
    const due = this.store.solidifiedStrategiesSnapshot()
      .filter(strategy => strategyDueForReview(strategy, now))
      .filter(strategy => now - (strategy.lastReviewedAt ?? 0) >= CognitivePipelineService.REVIEW_RECHECK_COOLDOWN_MS)
      .slice(0, CognitivePipelineService.REVIEW_PASS_CAP)
    let reviewed = 0
    let anchorHeld = 0
    let anchorFailed = 0
    let unverified = 0
    for (const strategy of due) {
      const verdict = await this.verifyStrategyAnchor(strategy)
      this.store.foldStrategyRecheck(strategy.strategyId, verdict)
      this.store.recordStrategyReview(strategy.strategyId)
      reviewed += 1
      if (verdict === true) anchorHeld += 1
      else if (verdict === false) anchorFailed += 1
      else unverified += 1
    }
    if (reviewed > 0) await this.store.flush()
    return { reviewed, anchorHeld, anchorFailed, unverified }
  }

  /** Re-check one strategy's verification anchor. A real command run only
   * when command execution is enabled (acceptanceCommandExecution) AND the
   * anchor reads as an ASCII shell command line; anything else is
   * unverifiable here and yields null (never a false "held").
   * @param strategy - the strategy under review.
   * @returns true = anchor held, false = anchor failed, null = unverifiable.
   */
  private async verifyStrategyAnchor(strategy: SolidifiedStrategy): Promise<boolean | null> {
    if (!this.resolved.acceptanceCommandExecution) return null
    const anchor = strategy.verificationAnchor.trim()
    if (anchor.length === 0 || /[\u4e00-\u9fa5；：，。！？]/.test(anchor)) return null
    try {
      const code = await this.runCommandExitCode(anchor, this.resolved.acceptanceCommandTimeoutMs)
  ...
  async offlineConsolidation(): Promise<{
    ...
    // Drift re-verification rides the same idle cadence: due strategies have
    // their verification anchors re-checked (see runStrategyReviewPass).
    void this.runStrategyReviewPass().catch((error: unknown) => {
      this.ctx.logger.warn(`cognitive-pipeline: strategy review pass failed: ${String(error)}`)
    })
