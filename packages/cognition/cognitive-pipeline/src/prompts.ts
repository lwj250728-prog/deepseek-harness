...
export const SAR_SYSTEM_PROMPT = [
  ...
  '4. 若原文自带结构标记（"情境："、"动作："、"行动："、"结果："及其英文形式），标记是权威切分：situation 只取"情境"标记下到下一个标记前的内容，action 只取"动作/行动"标记下的内容，outcome 只取"结果"标记下的内容。禁止把某个标记下的内容填进另一个字段，禁止在字段开头保留标记文字本身。',
  '5. 三个字段都必须是原文的编码，不得凭空添加原文没有的信息。outcome_utility_score 必须反映原文实际陈述的成败与代价，不得因为难以判断就一律给 5（中性分数只用于原文确实无成败信号时）。',
