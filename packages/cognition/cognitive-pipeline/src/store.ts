...
export class CognitiveStore {
  ...
  /** Fold one drift re-check outcome into a strategy's validity ledger,
   * SEPARATE from usage: a re-check is not a use (hitCount stays). A failed
   * re-check means the anchor no longer holds NOW, so it flags rework
   * immediately (regardless of the historical use ratio) and records one
   * violation for history; a held re-check clears a prior rework flag (the
   * strategy recovered) without touching counts. A null verdict (unverifiable
   * without command execution) changes nothing here.
   * @param strategyId - the strategy id.
   * @param held - true = anchor held, false = anchor failed, null = unverified.
   * @returns the updated strategy, or undefined when unknown.
   */
  foldStrategyRecheck(strategyId: string, held: boolean | null): SolidifiedStrategy | undefined {
    const strategy = this.solidifiedStrategies.get(strategyId)
    if (strategy === undefined || held === null) return strategy
    const violatedCount = strategy.violatedCount + (held ? 0 : 1)
    const next: SolidifiedStrategy = {
      ...strategy,
      violatedCount,
      reworkNeeded: held ? false : true,
      updatedAt: Date.now(),
    }
    this.solidifiedStrategies.set(strategyId, next)
    this.enqueue('solidified_strategies.json', [...this.solidifiedStrategies.values()])
    return next
  }

  /** Record one review of a solidified strategy: refreshes the activation
   * (reviewCount), without touching the usage ledger. The drift
   * re-verification executor calls this after an anchor check settles.
   * @param strategyId - the strategy id.
   * @returns the updated strategy, or undefined when unknown.
   */
  recordStrategyReview(strategyId: string): SolidifiedStrategy | undefined {
    const strategy = this.solidifiedStrategies.get(strategyId)
    if (strategy === undefined) return undefined
    const next: SolidifiedStrategy = {
      ...strategy,
      lastReviewedAt: Date.now(),
      reviewCount: (strategy.reviewCount ?? 0) + 1,
      updatedAt: Date.now(),
    }
    this.solidifiedStrategies.set(strategyId, next)
    this.enqueue('solidified_strategies.json', [...this.solidifiedStrategies.values()])
  ...
  /** Record one review of an experience: refreshes its activation clock and
   * review count through the ordinary update path (persisted with the
   * experience ledger).
   * @param expId - the experience id.
   * @returns the updated experience.
   */
  recordExperienceReview(expId: string): Experience {
    const current = this.experiences.get(expId)
    if (current === undefined) {
      throw new Error(`cognitive-pipeline: experience "${expId}" not found`)
    }
    return this.updateExperience(expId, {
      lastReviewedAt: Date.now(),
