...
*/
function situationVector(text) {
	return bagVector(tokenize(text).map((token) => `${SITUATION_TOKEN_PREFIX}${token}`), 384);
}
/** Default z-score threshold of the disequilibrium gate (μ±2σ, a ~5% tail
* event under the normal approximation). */
const DEFAULT_DISEQUILIBRIUM_Z = 2;
...
const DEFAULT_DISEQUILIBRIUM_MIN_SAMPLES = 3;
...
*/
function disequilibriumOf(prior, quality, zThreshold = 2, minSamples = 3) {
	...
	return {
		zScore,
		disequilibrated: zScore >= zThreshold
	};
}
/**
...
*/
function variantConvergence(settlements, adoptMinQuality = 7, rejectMaxMean = 4, minSamples = 3) {
...
//#endregion
export { symptomOverlap as _, SYMPTOM_MARKERS as a, variantConvergence as b, cosine as c, isPositiveOutcome as d, normalize as f, situationVector as g, signatureHash as h, OUTCOME_VECTOR_DIM as i, disequilibriumOf as l, outcomeVector as m, DEFAULT_DISEQUILIBRIUM_MIN_SAMPLES as n, UTILITY_SLOTS as o, outcomePolarity as p, DEFAULT_DISEQUILIBRIUM_Z as r, actionVector as s, ACTION_VECTOR_DIM as t, hashToken as u, tokenize as v, utilityScore as y };
