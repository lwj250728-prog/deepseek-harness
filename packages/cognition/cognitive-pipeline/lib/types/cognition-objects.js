 ...
 */
import { outcomePolarity, tokenize } from "./vectorizer.js";
import { STOP_WORDS } from "./triggers.js";
 ...
 */
export function assembleChain(chainId, goal, anchorSessionId, members, previous, now) {
    const ordered = [...members].sort((a, b) => (a.sequence ?? Number.MAX_SAFE_INTEGER) - (b.sequence ?? Number.MAX_SAFE_INTEGER)
    ...
    for (const member of ordered) {
        ...
        if (polarity === 'negative' || isDelegation) {
            ...
            sequence += 1;
            if (isDelegation)
                delegationIds.push(member.parentNodeId);
        }
        else {
    ...
    const memberSetChanged = previous !== undefined && (previous.memberExpIds.length !== memberExpIds.length
 ...
 */
export function childChainIdsOf(chain, experiences) {
    const receipts = new Set(chain.delegationNodeIds);
    if (receipts.size === 0)
        return [];
    const roots = new Map();
    for (const exp of experiences) {
        if (exp.chainId === undefined || exp.chainId === chain.chainId)
            continue;
    ...
    for (const [chainId, root] of roots) {
        if (root.parentNodeId !== undefined && receipts.has(root.parentNodeId))
            children.add(chainId);
    }
    return [...children];
}
/**
...
export class ChainObjectKind {
    ...
    project(store, config) {
        ...
        for (const [chainId, members] of byChain) {
            if (members.length < config.chainMinMembers)
                continue;
            const previous = store.getChain(chainId);
            const first = members[0];
            const assembled = assembleChain(chainId, previous?.goal ?? (first === undefined ? chainId : first.sar.situation.slice(0, 80)), previous?.anchorSessionId ?? null, members, previous, now);
 ...
 */
export function chainSignature(chain) {
 ...
 */
export class ChainPatternObjectKind {
    name = 'chain-pattern';
    ...
    project(store, config) {
        ...
        for (const [signature, group] of bySignature) {
            if (group.length < config.chainPatternMinMembers)
                continue;
    ...
    measure(store, objectId, _feedback) {
        ...
        for (const pattern of store.chainPatternsSnapshot()) {
            if (pattern.chainIds.includes(objectId))
                store.recomputeChainPatternStats(pattern.patternId);
        }
    }
    reinforce(_store, _config, build) {
        return build;
...
//# sourceMappingURL=cognition-objects.js.map