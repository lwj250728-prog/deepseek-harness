 ...
 */
import { CognitiveLoopRegistry, CognitivePipelineService, Config, } from "./service.js";
...
export { isTaskRestatement } from "./task-restatement.js";
...
export { findToolCallEvidence } from "./log-evidence.js";
...
export async function apply(ctx, config = {}) {
    ...
    ctx.on('session/event', (session, event) => {
        ...
        void service.summarizeTurn(session.id, episode).then((summary) => {
            if (summary !== null)
                session.append('cognition/turn-summary', summary);
        }).catch((error) => {
