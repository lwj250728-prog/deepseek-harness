...
import { ACCUMULATE_SYSTEM_PROMPT, CALIBRATION_SYSTEM_PROMPT, DERIVE_REFERENCE_SYSTEM_PROMPT, DISTILL_SYSTEM_PROMPT, PROPOSE_ACCEPTANCE_SYSTEM_PROMPT, PROPOSE_DISCRIMINANT_AXES_SYSTEM_PROMPT, PROPOSE_TRIGGER_JUMPS_SYSTEM_PROMPT, REFINE_RETRIEVAL_SYSTEM_PROMPT, frameAccumulateInput, frameCalibrationInput, frameDeriveReferenceInput, frameDiscriminantAxesInput, frameDistillInput, frameOodInput, frameProposeAcceptanceInput, frameProposeTriggerJumpsInput, frameReconstructInput, frameRefineRetrievalInput, frameSarInput, frameVariantInput, OOD_REVIEW_SYSTEM_PROMPT, RECONSTRUCT_SYSTEM_PROMPT, SAR_SYSTEM_PROMPT, VARIANT_SYSTEM_PROMPT, } from "./prompts.js";
import { SYMPTOM_MARKERS, tokenize } from "./vectorizer.js";
 ...
 */
export function hasExplicitRoute(route) {
...
export function resolveRoute(route) {
    ...
    if (provider === undefined || model === undefined || provider.length === 0 || model.length === 0) {
        throw new CognitivePipelineError('cognitive-pipeline: provider and model must be supplied together as non-empty strings', 'INVALID_LLM_ROUTE');
    }
...
function asObject(value, label) {
    if (typeof value !== 'object' || value === null) {
        throw new CognitivePipelineError(`cognitive-pipeline: ${label} output must be a JSON object`, 'LLM_SCHEMA_FAILED');
    }
    return value;
}
...
function finishError(finish) {
    switch (finish.kind) {
        ...
        case 'error':
        case 'aborted':
            return new CognitivePipelineError(`cognitive-pipeline: model call failed: ${finish.failure.message}`, finish.failure.code);
        case 'max-tokens':
...
async function drainText(ctx, options, maxTokens) {
    ...
    if (text.trim().length === 0) {
        throw new CognitivePipelineError(`cognitive-pipeline: model produced no text (maxTokens=${maxTokens})`, 'EMPTY_LLM_OUTPUT');
    }
    return text;
}
 ...
 */
export async function extractSar(ctx, route, rawText, options) {
    ...
    try {
        ...
        const keywords = Array.isArray(parsed.action_keywords)
            ? parsed.action_keywords.filter((keyword) => typeof keyword === 'string').slice(0, 16)
            : [];
 ...
 */
export function oodReviewFallback(isKnown) {
    return {
        isKnown,
        confidenceScore: 50,
 ...
 */
export async function reviewOod(ctx, route, action, topActions, mathSaysKnown, options) {
    if (!hasExplicitRoute(route))
 ...
 */
export function calibrationFallback(positiveCount, negativeCount) {
 ...
 */
export async function calibrate(ctx, route, input, options) {
    if (!hasExplicitRoute(route)) {
    ...
    try {
        ...
        const factors = Array.isArray(parsed.risk_factors)
            ? parsed.risk_factors.filter((factor) => typeof factor === 'string').slice(0, 5)
            : [];
 ...
 */
export function reconstructFallback(groups, summaryShort) {
    const newClusters = groups.map((group, index) => {
 ...
 */
export async function reconstructTaxonomy(ctx, route, samples, groups, summaryShort, options) {
    if (!hasExplicitRoute(route))
    ...
    try {
        ...
        for (const raw of rawClusters) {
            if (typeof raw !== 'object' || raw === null)
                continue;
            const cluster = raw;
            if (typeof cluster.cluster_name !== 'string' || typeof cluster.decision_rule !== 'string')
                continue;
            const range = cluster.expected_utility_range;
            const evidence = Array.isArray(cluster.supporting_evidence_ids)
                ? cluster.supporting_evidence_ids.filter((id) => typeof id === 'string')
                : [];
 ...
 */
export function accumulationFallback() {
 ...
 */
export async function evaluateAccumulation(ctx, route, episode, similar, options) {
    if (!hasExplicitRoute(route))
 ...
 * @returns the rejection decision.
 */
export function deriveReferenceFallback() {
 ...
 */
export async function deriveReference(ctx, route, query, similar, options) {
    if (!hasExplicitRoute(route))
 ...
 */
export function refineRetrievalFallback() {
 ...
 */
export async function refineRetrieval(ctx, route, query, candidates, options) {
    if (!hasExplicitRoute(route))
 ...
 */
export async function generateVariants(ctx, route, input, options) {
    if (!hasExplicitRoute(route))
        return [];
    try {
 ...
 */
export function proposeAcceptanceFallback() {
    return { proposals: [] };
}
/**
 ...
 */
export async function proposeAcceptanceUpdates(ctx, route, flagged, deviationMeta, options) {
    if (!hasExplicitRoute(route))
        return proposeAcceptanceFallback();
    try {
        const framed = frameProposeAcceptanceInput(flagged, deviationMeta);
        const parsed = asObject(await callJson(ctx, route, PROPOSE_ACCEPTANCE_SYSTEM_PROMPT, framed, {
            ...options,
            maxTokens: 600,
        }), 'acceptance-proposal');
 ...
 * @returns the empty-proposal decision.
 */
export function triggerJumpsFallback() {
    return { jumps: [] };
}
/**
 ...
 */
export async function proposeTriggerJumps(ctx, route, input, options) {
    if (!hasExplicitRoute(route))
    ...
    for (let draw = 0; draw < maxDraws; draw += 1) {
        try {
            ...
            const parsed = asObject(await callJson(ctx, route, PROPOSE_TRIGGER_JUMPS_SYSTEM_PROMPT, framed, {
                ...options,
            ...
            for (const raw of rawJumps) {
                ...
                const variants = Array.isArray(entry.variants)
                    ? entry.variants.filter((variant) => typeof variant === 'string' && variant.length > 0)
                    : [];
 ...
 */
export function distillFallback() {
 ...
 */
export async function distillChainPrinciple(ctx, route, input, options) {
    if (!hasExplicitRoute(route))
 ...
 */
export function discriminantAxesFallback() {
    return { axes: [] };
}
/**
 ...
 */
export async function proposeDiscriminantAxes(ctx, route, input, options) {
    if (!hasExplicitRoute(route))
        ...
        try {
            ...
            for (const raw of rawAxes) {
                ...
                const terms = Array.isArray(entry.terms)
                    ? entry.terms.filter((term) => typeof term === 'string' && term.length > 0)
                    : [];
