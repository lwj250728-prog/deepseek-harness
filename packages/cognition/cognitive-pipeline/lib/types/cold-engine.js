 ...
 */
import { reconstructTaxonomy } from "./llm.js";
import { cosine, outcomePolarity, outcomeVector, situationVector, utilityScore } from "./vectorizer.js";
...
function meanUtility(items) {
    if (items.length === 0)
        return { materialGain: 5, emotionalValence: 5, energyCost: 5 };
    let materialGain = 0;
...
function clusterVectorOf(exp, source) {
    if (source === 'embedding' && exp.embedding !== undefined)
        return exp.embedding;
    const samples = exp.settlements ?? [];
    if (samples.length === 0)
        return exp.outcomeVector;
...
function verifyEvidence(candidate, byId, minCount, maxDistance, source) {
    ...
    const evidence = candidate.evidenceIds.map(id => byId.get(id)).filter((exp) => exp !== undefined);
...
export class ColdEngine {
    ...
    async runRebuild(scope, sessionId, signal) {
        ...
        for (let attempt = 0; attempt <= retries; attempt += 1) {
            const reconstruct = await reconstructTaxonomy(this.ctx, this.route, train, groupsWithUtility, summaryShort, { sessionId, signal });
        ...
        if (validation.length > 0) {
            ...
            validation.forEach((exp, index) => {
                if (!Number.isFinite(exp.sar.outcomeUtility.materialGain))
                    return;
    ...
    sample(all, scope) {
        ...
        if (scope === 'local') {
            ...
            for (const cluster of clusters) {
                if (worst === undefined || cluster.cumPredictionError > worst.cumPredictionError)
                    worst = cluster;
            }
            if (worst !== undefined) {
    ...
    cap(candidates, total) {
        ...
        const sorted = [...candidates].sort((a, b) => (b.cumulativeError + (b.predictionError ?? 0)) - (a.cumulativeError + (a.predictionError ?? 0)));
    ...
    fallbackCandidates(groups, byId) {
        return groups.map((group, index) => {
            const evidence = group.evidenceIds.map(id => byId.get(id)).filter((exp) => exp !== undefined);
    ...
    clusterViews(all, clusters) {
        ...
        const views = [];
        for (const cluster of clusters) {
            const evidence = cluster.supportingEvidenceIds.map(id => byId.get(id)).filter((exp) => exp !== undefined);
...
//# sourceMappingURL=cold-engine.js.map