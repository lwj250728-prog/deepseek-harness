...
export const STATIC_TRIGGERS = new Set([
 ...
 */
export function importanceOf(exp) {
    ...
    if (utility < 0.01 && exp.hitCount === 0 && (gain >= 5 && valence >= 5 && cost <= 5))
        return 0;
 ...
 */
export function deriveTriggerWords(service) {
    ...
    for (const exp of service.store.experiencesSnapshot()) {
        ...
        for (const token of tokens) {
            if (STOP_WORDS.has(token) || STATIC_TRIGGERS.has(token))
                continue;
 ...
 */
export function emptyJumpAccumulator() {
    return new Map();
}
/**
 ...
 */
export function accumulateTriggerJumps(service, accumulator, derived) {
    ...
    for (const exp of service.store.experiencesSnapshot()) {
        ...
        for (const trigger of STATIC_TRIGGERS) {
            if (text.includes(trigger))
                presentTriggers.add(trigger);
        }
        for (const token of tokens) {
            if (derivedTokens.has(token))
                presentTriggers.add(token);
        }
        for (const trigger of presentTriggers) {
            for (const token of tokens) {
                if (token === trigger || STOP_WORDS.has(token) || STATIC_TRIGGERS.has(token))
                    continue;
                const byTrigger = accumulator.get(token) ?? new Map();
 ...
 */
export function jumpVocabulary(text) {
