...
import { EmbeddingScorer } from "./embedding.js";
import { HotEngine } from "./hot-engine.js";
import { CognitivePipelineError, deriveReference, distillChainPrinciple, evaluateAccumulation, extractSar, generateVariants, hasExplicitRoute, proposeAcceptanceUpdates, proposeDiscriminantAxes, proposeTriggerJumps, resolveRoute, } from "./llm.js";
import { cognitionPrefix } from "./prompts.js";
import { CognitiveStore } from "./store.js";
import { actionVector, cosine, outcomePolarity, outcomeVector, signatureHash, tokenize, utilityScore, variantConvergence } from "./vectorizer.js";
import { STATIC_TRIGGERS, STOP_WORDS, accumulateTriggerJumps, deriveTriggerWords, emptyJumpAccumulator, importanceOf, } from "./triggers.js";
import { ChainObjectKind, ChainPatternObjectKind, assembleChain, childChainIdsOf, } from "./cognition-objects.js";
import { isTaskRestatement } from "./task-restatement.js";
...
export class CognitiveLoopRegistry {
    ...
    register(spec) {
        ...
        if (!/^[a-z][a-z0-9-]*$/.test(spec.name)) {
            throw new CognitivePipelineError('cognitive-pipeline: loop name must match ^[a-z][a-z0-9-]*$ (lowercase, hyphen-separated)', 'INVALID_LOOP_NAME');
        }
    ...
    async requestExecution(request) {
        ...
        if (spec?.execution === undefined || !request.approved)
            return [];
        const receipts = [];
...
export class CognitivePipelineService extends Service {
    ...
    async deriveReference(input, call) {
        if (input.situation.trim().length === 0 || input.action.trim().length === 0) {
            throw new CognitivePipelineError('cognitive-pipeline: situation and action must not be empty', 'EMPTY_DERIVE_REFERENCE_INPUT');
        }
        ...
        if (!decision.shouldDerive || decision.sar === null)
            return null;
        const sar = {
    ...
    async accumulateTurn(episode, call) {
        ...
        const top = similar[0];
        if (top !== undefined && !episode.failed && top.polarity === 'positive')
            return null;
        ...
        if (isTaskRestatement({ sar: decision.sar }))
            return null;
        const expId = this.store.nextExpId();
        const sar = {
            situation: decision.sar.situation,
    ...
    async report(input, call) {
        ...
        if (prediction.exploredActionHash !== null) {
            this.store.validateExploration(prediction.exploredActionHash, error, this.resolved.exploreValidationLearningRate, this.resolved.exploreValidationErrorThreshold);
        }
    ...
    async decideLoop(name, decision, situation, call) {
        if (!this.loops.has(name)) {
            throw new CognitivePipelineError(`cognitive-pipeline: loop "${name}" is not registered (register it first)`, 'INVALID_LOOP_NAME');
        }
        return this.predict({
     ...
     */
    async decideAndExecute(name, decision, situation, threshold = 0.55, call) {
    ...
    async settleExecution(receiptId, outcomeText, outcomeQuality, status = 'executed', call) {
        ...
        if (receipt === undefined) {
            throw new CognitivePipelineError(`cognitive-pipeline: execution receipt "${receiptId}" not found`, 'EXECUTION_RECEIPT_NOT_FOUND');
        }
        if (receipt.rejected) {
            throw new CognitivePipelineError(`cognitive-pipeline: receipt "${receiptId}" was refused by the sink and cannot be settled`, 'EXECUTION_RECEIPT_REFUSED');
        }
        if (receipt.settledAt !== null) {
            throw new CognitivePipelineError(`cognitive-pipeline: execution receipt "${receiptId}" is already settled`, 'EXECUTION_RECEIPT_ALREADY_SETTLED');
        }
        ...
        if (settled === undefined) {
            ...
            throw new CognitivePipelineError(`cognitive-pipeline: execution receipt "${receiptId}" disappeared during settlement`, 'EXECUTION_RECEIPT_NOT_FOUND');
        }
    ...
    async defineAcceptanceCheck(input) {
        ...
        if (criterion.length === 0 || trigger.length === 0 || evidenceHint.length === 0) {
            throw new CognitivePipelineError('cognitive-pipeline: criterion, trigger, and evidenceHint must not be empty', 'EMPTY_ACCEPTANCE_INPUT');
        }
        const now = Date.now();
        const check = {
    ...
    async auditClaim(input) {
        ...
        for (const check of applied) {
            ...
            if (passed)
                satisfiedCheckIds.push(check.checkId);
            else
                violatedCheckIds.push(check.checkId);
            ...
            if (crossedNow && !crossedBefore)
                firstCrossingChecks.push(updated);
        }
    ...
    async updateAcceptanceCheck(input) {
        ...
        if (current === undefined) {
            throw new CognitivePipelineError(`cognitive-pipeline: acceptance check "${input.checkId}" not found`, 'ACCEPTANCE_CHECK_NOT_FOUND');
        }
        if (current.status === 'retired') {
            throw new CognitivePipelineError(`cognitive-pipeline: acceptance check "${input.checkId}" is retired and frozen`, 'ACCEPTANCE_CHECK_RETIRED');
        }
        ...
        if (criterion === undefined && evidenceHint === undefined && trigger === undefined) {
            throw new CognitivePipelineError('cognitive-pipeline: update needs criterion, evidenceHint, trigger, or retire', 'EMPTY_ACCEPTANCE_UPDATE');
        }
        if (criterion !== undefined && criterion.length === 0) {
            throw new CognitivePipelineError('cognitive-pipeline: criterion must not be empty', 'EMPTY_ACCEPTANCE_UPDATE');
        }
        if (evidenceHint !== undefined && evidenceHint.length === 0) {
            throw new CognitivePipelineError('cognitive-pipeline: evidenceHint must not be empty', 'EMPTY_ACCEPTANCE_UPDATE');
        }
        if (trigger !== undefined && trigger.length === 0) {
            throw new CognitivePipelineError('cognitive-pipeline: trigger must not be empty', 'EMPTY_ACCEPTANCE_UPDATE');
        }
    ...
    async proposeAcceptanceUpdate(call) {
        const active = this.store.acceptanceSnapshot().filter(check => check.status === 'active');
        const flagged = active.filter(check => check.invokedCount >= this.resolved.acceptanceMinEvidenceCount
            && check.violatedCount / check.invokedCount >= this.resolved.acceptanceDeviationThreshold);
        if (flagged.length === 0)
            return { flagged: [], proposals: [], applied: [] };
        ...
        for (const proposal of decision.proposals) {
            ...
            if (!flaggedIds.has(proposal.checkId))
                continue;
            if (proposal.rationale.trim().length === 0)
                continue;
            if (proposal.action === 'rewrite' && (proposal.criterion?.trim().length ?? 0) === 0)
                continue;
    ...
    async runCommandExitCode(command, timeoutMs) {
        ...
        if (shell === undefined) {
            throw new CognitivePipelineError('cognitive-pipeline: command anchors require the shell capability (ctx.shell) to be mounted in the composition', 'SHELL_CAPABILITY_UNAVAILABLE');
        }
    ...
    async learnTriggerJumps(call) {
        ...
        if (hasExplicitRoute(this.resolved.route)) {
            ...
            const situationsByWord = new Map();
            for (const exp of this.store.experiencesSnapshot()) {
                ...
                for (const word of focusWords) {
                    ...
                    if (bucket.length < 2)
                        bucket.push(exp.sar.situation.slice(0, 60));
            ...
            for (const proposal of decision.jumps) {
                if (!STATIC_TRIGGERS.has(proposal.trigger) && !derived.has(proposal.trigger))
                    continue;
                for (const variant of proposal.variants) {
                    if (variant === proposal.trigger || STOP_WORDS.has(variant) || jumps.has(variant))
                        continue;
        ...
        for (const record of pending) {
            ...
            if (record.strategyId !== null) {
                ...
                if (after !== undefined && (before?.reworkNeeded === true || after.reworkNeeded)) {
                    const stranded = this.store.variantsSnapshot().some(candidate => candidate.sourceStrategyId === record.strategyId
    ...
    async rebuildCognitionObject(name) {
        const kind = this.objectKinds.get(name);
        if (kind === undefined) {
            throw new CognitivePipelineError(`cognitive-pipeline: cognition object kind "${name}" is not registered`, 'COGNITION_OBJECT_NOT_FOUND');
        }
    ...
    async consolidateChain(chainId, goal) {
        ...
        if (members.length < this.resolved.chainMinMembers)
            return null;
        const previous = this.store.getChain(chainId);
        const first = members[0];
        const chain = assembleChain(chainId, goal?.trim() || previous?.goal || (first === undefined ? chainId : first.sar.situation.slice(0, 80)), previous?.anchorSessionId ?? null, members, previous, Date.now());
        ...
        const memberSetChanged = previous !== undefined && (previous.memberExpIds.length !== withChildren.memberExpIds.length
    ...
    async extractDiscriminantAxes(call) {
        if (!hasExplicitRoute(this.resolved.route))
            return { axesCount: 0, clustersExamined: 0 };
    ...
    async generateStrategyVariants(strategyId) {
        ...
        for (const candidate of candidates)
            this.store.addVariantCandidate(candidate);
        if (candidates.length > 0)
            await this.store.flush();
        return candidates;
    }
     ...
     */
    async pruneExperiences(zeroCitationRetentionMs = 30 * 24 * 60 * 60 * 1000) {
        const now = Date.now();
        const removed = [];
        for (const exp of this.store.experiencesSnapshot()) {
            if ((exp.citationCount ?? 0) > 0)
                continue;
            if (now - exp.timestamp < zeroCitationRetentionMs)
                continue;
            if (this.store.removeExperience(exp.expId))
                removed.push(exp.expId);
        }
        if (removed.length > 0)
            await this.store.flush();
        return removed;
    ...
    async offlineConsolidation() {
        ...
        if (last !== null && now - last < this.resolved.offlineConsolidationIntervalMs)
            return null;
        this.lastOfflineConsolidation = now;
        const consolidatedChains = [];
        const chainIds = new Set();
        for (const exp of this.store.experiencesSnapshot()) {
            if (exp.chainId !== undefined)
                chainIds.add(exp.chainId);
        }
        for (const chainId of chainIds) {
            const chain = await this.consolidateChain(chainId);
            if (chain !== null)
                consolidatedChains.push(chain.chainId);
        }
    ...
    async foldVariantFeedback(action, outcomeQuality) {
        ...
        for (const candidate of this.store.variantsSnapshot()) {
            if (candidate.status !== 'proposed' && candidate.status !== 'testing')
                continue;
     ...
     */
    exploreChainNeighbors(expId, minCosine = 0.3, limit = 5) {
