 ...
 */
export function situationVector(text) {
 ...
 */
export function utilityScore(utility) {
 ...
 */
export function isPositiveOutcome(utility) {
 ...
 */
export function disequilibriumOf(prior, quality, zThreshold = DEFAULT_DISEQUILIBRIUM_Z, minSamples = DEFAULT_DISEQUILIBRIUM_MIN_SAMPLES) {
 ...
 */
export function variantConvergence(settlements, adoptMinQuality = 7, rejectMaxMean = 4, minSamples = 3) {
    if (settlements.length < minSamples)
        return 'insufficient';
    const mean = settlements.reduce((sum, sample) => sum + sample.quality, 0) / settlements.length;
    if (settlements.every(sample => sample.quality >= adoptMinQuality - 1) && mean >= adoptMinQuality)
        return 'adopt';
    if (mean <= rejectMaxMean)
        return 'reject';
    return 'keep-testing';
}
 ...
 */
export function symptomOverlap(query, text) {
    const lower = text.toLowerCase();
    let matched = 0;
    let present = 0;
    for (const marker of SYMPTOM_MARKERS) {
        if (!query.toLowerCase().includes(marker))
            continue;
        present += 1;
 ...
 */
export function outcomePolarity(utility) {
 ...
 */
export function actionVector(action, keywords) {
 ...
 */
export function outcomeVector(utility, outcomeText) {
    const vector = new Array(OUTCOME_VECTOR_DIM).fill(0);
 ...
 */
export function signatureHash(action) {
