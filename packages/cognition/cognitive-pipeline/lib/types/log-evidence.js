 ...
 */
export function findToolCallEvidence(session, toolName) {
    const events = session.events;
    for (let index = events.length - 1; index >= 0; index -= 1) {
        const event = events[index];
        if (event === undefined || event.type !== 'tool/call')
            continue;
        const call = event.data;
        if (typeof call.name !== 'string' || call.name !== toolName)
            continue;
        ...
        for (let resultIndex = index + 1; resultIndex < events.length; resultIndex += 1) {
            ...
            if (resultEvent === undefined || resultEvent.type !== 'tool/result')
                continue;
            ...
            if (callId !== '' && sourceCallId !== callId && blockCallId !== callId)
                continue;
