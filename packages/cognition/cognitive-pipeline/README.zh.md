...
- **经验链 (goal-anchored chains)** — 第五个派生认知对象，也是**派生对象抽象**（`CognitionObjectKind`：project/persist/measure/reinforce/expose——新增特殊经验层只需一次声明，通用驱动 `rebuild_cognition_object` 服务所有种类）的首个声明式实例。`consolidate_chain` 从打上链标签的经验（经验上的 `chainId`/`parentNodeId`/`sequence`）组装目标锚定的因果骨架：失败步与跨 agent 委派节点（回执）保持结构步，例行成功步坍缩为有界摘要（记忆组织在惊喜周围），链级引用率（注入链被模型引用即 cited）度量整段目标执行是否值得记忆。链在离线时巩固（目标完成/agent 空闲——巩固的工程对应），由 `chainMinMembers` 证据闸门把关，持久化于 `chains.json`，经 `chainExpose` 以结构化步骤呈现给注入路径。链构成**目标树**：委派子目标的链挂在委派链之下（其根成员引用父链回执），`chainChildren`/`chainTreeExpose` 暴露该结构以支持目标结构化扩散——命中父链时可下钻到子目标结果。**经验蒸馏 (chain principle distillation)**：存在显式 LLM 路由时，巩固还会请路由从链成员（失败优先）提炼**一条**可复用决策原则——EvolveR 式"从经验到原则"——存为链上的 `distilledPrinciple` 并在 `chainExpose` 呈现；无路由则不蒸馏（宁缺毋滥），成员集未变时保留旧原则、不产生新的 LLM 调用。
- **链模式 (chain patterns)** — 第六个派生认知对象，也是该抽象的首个**递归**消费者：模式从链表投影，正如链从经验投影。共享结构签名（粗粒度目标域 + 步极性序列 + **因果断裂轴**：成员是否自反地杀过自身宿主，后缀 `~自反`）的链聚合为一种周期性目标执行模式（TOPS 类比：从相似 MOP 提炼跨情境主题模式），由 `chainPatternMinMembers` 闸门把关，持久化于 `chain_patterns.json`。因果断裂轴是跨域主题投影器："自反中断→外部见证重建"在不同目标域中反复出现，因此不同域但都携带断裂点的链共享签名后缀，可聚合为一个主题。度量效用从成员链的引用统计经同一通用 measure 分派聚合，因此模式的引用率可追溯地度量分组是否有用。用 `rebuild_cognition_object('chain-pattern')` 重建。
...
- **固化策略 (solidified strategies)** — 把反复成功的操作（如"重启 DSH = 调用 `scripts/dsh-web-autorestart.ps1`"）从 SAR 记忆提升为可复用、**自我验证的规则**。四部分使其免受环境漂移侵害：**动作**（具体操作）、**验收锚点**（机器可核验的验收如 `selfPerformed=true`——每次使用都重查的漂移传感器）、**生命周期**（invoked/violated 账本，偏离门在环境漂移时把策略标记为需重学而非静默失效）、**前置校验**（执行前验证的条件，把漂移检测从事后提前到事前）。当注入检索到链接到某固化策略链的经验时，注入**策略块**而非散落经验——收敛形态直接告诉执行者跑什么、怎么验证成功。
...
| `autoAccumulate` | `false` | 完成的轮次自动沉淀为经验，由 LLM 路由判断是否值得（纯聊天不进入门）。自反轮次（调用杀死/重启自身宿主进程的工具）会为闸门加注——杀进程后的因果链在本会话内不可观测，重构的行动被标记为推测性，未经外部见证不得断言为事实。上下文压缩摘要同样进入同一闸门（记忆锚定写回）：`compaction/summary` 事件把长弧段折叠为一条摘要时，该弧段每个压缩 id 至多提交一次给闸门 |
