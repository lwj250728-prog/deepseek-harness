...
- **热环路 (hot loop)** — `predict_outcome`: retrieves the top-K similar past actions, computes the OOD signals (`Top1 相似度 < 0.65`, `Top1-Top3 方差 < 0.1` (ambiguous, not near-exact), `Strangeness Index > 1.5`), and routes to the familiar path (five-layer calibration) or the novel path (scratchpad trial strategy with a `⚠️ 全新现象` marker).
- **五层校准 (five-layer calibration)** — frequency-prior prompt injection, sample-size shrinkage `P_cal = (k/(k+50))·P_raw + (50/(k+50))·0.5`, minimum-width 80% confidence interval, adversarial risk-factor listing, and lifetime bucket correction against empirical accuracy.
...
- **冷环路 (cold loop)** — `rebuild_taxonomy`: decay-weighted sampling `W = e^(−λ·Δt)` of high-error experiences (≤15% of the population, with a 32-sample floor), agglomerative clustering on **outcome utility vectors** (utility-first, not semantic), LLM causal anchoring with a hard ≥3-evidence constraint (backend-verified pairwise distance ≤ 0.85, hallucinated clusters rejected), and a sandbox backtest on the newest 20% that requires `Δerr ≤ −0.15` before atomic write-back.
...
```
The model can then use the five tools:
...
- `report_outcome` — feed the actual outcome back (optional `outcome_quality` 0–10), which updates calibration stats and may trigger an emergency local repair.
...
- **SAR extraction** — sentence-split fallback with neutral utility when no route is configured or the call fails.
...
`remember_experience`, `predict_outcome`, `report_outcome`, `rebuild_taxonomy`, and `inspect_memory` register with the tool registry (`ctx.tools.register` + `defineTool`); their schemas flow into the system-prompt tool catalog automatically, each tool returns one canonical JSON value mirrored into model-facing text by `output.render`, and the tool descriptions defined in `src/tools.ts` are the only static prompt text this package owns (surfaced in the generated [tool catalog](../../../docs/tool-catalog.md)).
...
- **Observed outcome quality** — feedback relies on the model's `outcome_quality` or an LLM SAR extraction; without either, the observed value falls back to the neutral 0.5 baseline, which under-reports error.
