...
```
The model can then use the eleven tools:
...
- `verify_claim` — audit one claim against the active criteria; violated criteria accumulate in the ledger and can flag rework.
...
### The eleven model-facing tools

#### What the model sees

`remember_experience`, `simulate_experience`, `reference_experience`, `predict_outcome`, `report_outcome`, `rebuild_taxonomy`, `inspect_memory`, `register_loop`, `define_acceptance_check`, `verify_claim`, and `update_acceptance_check` register with the tool registry (`ctx.tools.register` + `defineTool`); their schemas flow into the system-prompt tool catalog automatically, each tool returns one canonical JSON value mirrored into model-facing text by `output.render`, and the tool descriptions defined in `src/tools.ts` are the only static prompt text this package owns (surfaced in the generated [tool catalog](../../../docs/tool-catalog.md)).
