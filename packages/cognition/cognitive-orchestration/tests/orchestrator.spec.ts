...
import { waitForProvider } from '../src/index.ts'
import {
  delegationOutput,
  delegationTask,
  outputText,
...
describe('cognitive-orchestration waitForProvider', () => {
  it('resolves as soon as the provider appears within the wait', async () => {
    let registered: { name: string } | undefined
    const registry = { getProvider: () => registered }
    const appear = (async () => {
      await new Promise(resolve => setTimeout(resolve, 60))
      registered = { name: 'spawn' }
    })()
    const provider = await waitForProvider(registry as never, 'spawn', 2000)
    await appear
    expect(provider).toEqual({ name: 'spawn' })
  })

  it('returns undefined when the provider never appears before the wait elapses', async () => {
    const provider = await waitForProvider({ getProvider: () => undefined } as never, 'missing', 80)
