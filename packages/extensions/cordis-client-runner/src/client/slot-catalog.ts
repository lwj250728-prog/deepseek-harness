...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  },
  {
    ...
    keyDomain: 'fixed by the owner\'s key table { [Kind in ChatNodeKind]: { node: ChatNode<Kind> } }, already taken: assistant-step, cognition-summary, command, command-input, compaction, context, manual-compaction, model-retry, steering, tool-call, turn-error, turn-max-tokens, turn-tail, unknown, user, workflow-run',
    hookContext: 'string',
    ...
    occupants: [
      'client-ui-cognition CognitionSummaryNodeView key \'cognition-summary\'',
  ...
  },
  {
    ...
    occupants: [
      ...
      'client-ui-fiscal-map FiscalMapSection id \'fiscal-map\'',
  ...
  },
  {
    ...
    occupants: [
      'client-ui-goal-tree GoalTree id \'goal-tree\'',
  ...
  },
  {
    ...
    kind: 'single',
    scope: 'root',
    summary: 'The learning area seat between the browsing region and the foot: the cognitive pipeline\'s exploration task queue as a readable list.',
    doc: 'The learning area seat between the browsing region and the foot: the\ncognitive pipeline\'s exploration task queue as a readable list. Declared\nby this package\'s \'sidebar\' entry; ui-cognition registers its occupant.\nThe sidebar passes only its column state.',
    registerOptions: [],
    ownerProps: [
    ...
    declaredBy: 'an entry in \'sidebar\' (client-ui-sidebar), so it exists while that entry is mounted',
    occupants: [
      'client-ui-cognition LearningArea',
    ],
    replaceRisk: 'shadows-shipped-ui',
  ...
  },
  {
    ...
    kind: 'single',
    scope: 'root',
    summary: 'The digital-life strip above the browsing region: the designated main conversation\'s current state (chain head) and its recent situational trajectory.',
    doc: 'The digital-life strip above the browsing region: the designated main\nconversation\'s current state (chain head) and its recent situational\ntrajectory. Declared by this package\'s \'sidebar\' entry; ui-cognition\nregisters its occupant. The sidebar passes only its column state.',
    registerOptions: [],
    ownerProps: [
      '/**\n * Owner share of the browser hole — the only facts crossing the shell/region\n * boundary. Business data and actions arrive through the region\'s own inject.\n */\nexport interface SidebarSectionOwnerProps {\n  /** Shell fold-state output: wide renders the full browser, rail the icon column. */\n  wide: boolean\n  /** Rail icons request expansion; the browser rides the wide flip for focus. */\n  expandSidebar: () => void\n}',
    ],
