...
Step-level SAR experience priming for the cognitive pipeline. At every agent pre-step it extracts the current situation from the messages about to enter the model request, and **only when the situation carries a trigger** (a failed step, a help/explore/decide behavior word, a keyword derived from important stored experiences, or a learned jump word) does it retrieve situation-related experiences and inject the closest hits as reference context. After a failed step it recalls more aggressively — the "memory chaining" analogue: a failure is the strongest situation cue, so the previous setback surfaces related past experience for faster matching on the retry step. Every injection is recorded and settled at turn end: whether the model actually cited the injected experience folds back into the jump lexicon's measured utility, feeding its reinforcement loop.
...
主对话 / 子任务 每步开始前
  ...
  → 通过 → 固化策略优先：accepted 候选链接到已固化链 且 情境含策略目标域 → 注入【固化策略】块
  → 否则 → 注入【认知经验参考】块（source: cognitive-inject）+ 记录注入（recordInjection）
...
- **触发式注入 (trigger-gated injection)** — humans consult past experience when they fail, face something new, or make a high-stakes decision — not on routine small talk. Injection is gated the same way: after a failed step it always primes; otherwise the incoming messages must carry a trigger. **Static behavior triggers** (失败/报错/卡住/排查/怎么/如何/试试/风险/以前/遇到过/发布/部署/计划…) match as substrings; **SAR-derived triggers** are keywords of important stored experiences — tokens of the situation/action of high-utility, high-risk, or frequently-hit experiences accumulate their importance (|utilityScore|/15 + risk + frequency) into per-token weights, the top 60 survive normalized, and a summed trigger weight ≥ 0.6 primes injection. **Jump words** (the associative layer learned by `learn_trigger_jumps`) open the gate through synonym variants (发版 → 发布, 卡壳 → 卡住): each jump's contribution is scaled by `triggerJumpWeightScale`, so a single weak jump never opens it alone — two jumps or a jump plus a direct hit do. A message like "重启一下" (routine) never injects even when retrieval would find a literal weak hit; "帮我排查测试挂起" (help) and "打包插件到GitHub" (derived keywords of a failed-push experience) both prime.
- **引用率闭环 (citation-rate loop)** — every injection is recorded (`recordInjection` on the pipeline service) with the expIds, the trigger source, and the jump words that opened the gate; at turn end the closed turn's assistant text settles it (`settleInjectionCitations`): a reference to an injected expId means cited. The outcome folds into the contributing jump words' hit/cited ledger, and the next `learn_trigger_jumps` rebuild boosts well-cited jumps and prunes never-cited ones — the injection gate's own learning loop, measured on the same evidence-backed ruler as everything else in the pipeline.
- **延迟结算 (deferred settlement)** — settlement is NOT only at turn end: a self-reflexive operation (e.g. restarting the host) interrupts the turn, so `turn/end` never fires and the injection stays pending (`cited=null`). Real memory settles at later recall, not at the event: every pre-step first settles any still-pending injections for the session against the current step's text (the "host recovered, now I remember what I used" case). This keeps the strategy lifecycle drift sensor alive across restarts.
...
- **任务复述跳过 (task-restatement skip)** — a delegated task instruction that was auto-accumulated as an experience (its situation is the verbatim task text, its action merely re-states the delegation with no real tool trace) ranks at the top of every later injection for the same task, crowding out the experiences that actually hold the solution. Retrieval skips such records deterministically (the exp_155/168 lesson), so every executor — main conversation or subagent — sees the same genuine lessons and converges on the same reliable path.
- **否决门 (LLM veto gate)** — an over-threshold candidate is not automatically injected: when the pipeline has an LLM route, the template-7 refine route reads the situation and each candidate and judges whether it truly applies (a literal hit is not transferability). Every candidate the route accepts is injected (viewpoint coverage survives the veto — a failure + success pair both reach the model when both are judged transferable); each rejection records a note (bounded, `INJECT_VETO_MAX = 2`; the injected block notes how many were vetoed), and all-rejected suppresses injection entirely. Without a route the gate degrades to the threshold-only behavior. The veto runs **before** solidified-strategy promotion: a chain link alone never bypasses the applicability judgement.
- **固化策略优先 (solidified-strategy priority, after the veto)** — when the ACCEPTED experiences link to a chain that seeded a solidified strategy (the repeated-success promotion), the STRATEGY block is injected instead of the scattered experiences — the converged form that tells the executor exactly what to run and how to check it worked. Promotion requires BOTH the chain link AND the situation to carry the strategy's goal domain (`situation.includes(goalDomain)`): an experience that merely RECORDS the chain's past verification is not a request to run its strategy, so it falls through to the plain reference block.
- **上下文预热 (context prewarm)** — a short message ("重启") can match an unrelated experience by surface words (the exp_67 literal-overlap false positive), and the veto route judging that match sees only the isolated message — no way to know the real context. The plugin maintains a rolling prewarm summary per session (recent tool calls + assistant output, bounded, `PREWARM_MAX_ENTRIES`), and when a trigger fires, the veto-gate situation becomes `[当前会话正在进行] <prewarm> [当前消息] <message>` — so the LLM route judges applicability against what the session is ACTUALLY doing, sharply raising the veto success rate on literal-overlap false positives.
...
| `injectCooldownMs` | `600000` (10 min) | Same-session cooldown: an experience injected into this session within the window is not injected again (repeats of the same memory in one session are noise, not recall); `0` disables the cooldown |
| `review.enabled` | `false` | Main-session pre-input review: on the first step of a turn, for a root session with a substantive input, a review subagent synthesizes the retrieved experience and the 【过往经验回顾】 analysis replaces the raw blocks for that step |
| `review.provider` | `spawn` | Subagent provider the review child runs through |
| `review.minTextChars` | `60` | Genuine input shorter than this never spawns a review child |
| `review.cooldownMs` | `120000` (2 min) | Per-session floor between two reviews |
| `review.timeoutMs` | `45000` | Hard cap for one review; on expiry the raw blocks inject and the late output is discarded |
| `review.maxHits` | `3` | How many retrieved experiences the review prompt may cite |
