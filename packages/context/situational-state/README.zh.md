...
- **空链自举** — 当 pre-step 发现链表为空且 `autoBootstrap` 开启时，插件把会话开场情景作为首节点提交（`origin: bootstrap`），而非短路——链表无需手工工具调用或手编链文档即可自行启动。
- **回合末自检** — 当完成的回合确有工作（工具调用 ≥ `selfCheckMinToolCalls`）且链头年龄超过 `selfCheckMinHeadAgeMs` 时，插件把该回合最新的自述内容作为新节点提交（`origin: turn-end`）——链随对话推进而推进，无需模型记得调用提交工具。
- **压缩写回** — 当上下文压缩把一段被挤出的弧段折叠为摘要（会话的 `compaction/summary` 事件）时，插件以摘要文本提交一个链节点（`origin: compaction`）——压缩后的步骤与后续会话都能从基质恢复"当时正在做什么"，而非只靠被挤出的 surface。每个压缩 id 至多一个节点（重试幂等）；下一个 pre-step 注入刷新后的链头。
...
插件声明 `inject = ['agents', 'tools']`，加载器会在代理注册表与工具注册表之后挂载它，保证 commit/trace 工具注册成功（此前静默跳过注册、导致会话无法自建首节点的缺口已关闭）；`fs` 惰性读取用于链表持久化。
...
| `commit(agent, situation, nextUpdateAfterMs?, origin = 'tool'): Promise<SituationalStateCommitResult>` | 追加一个节点；带延迟时武装唤醒定时器。低于 `minUpdateDelayMs` 会向上钳制。`origin`（`tool`/`bootstrap`/`turn-end`/`compaction`）记入轨迹账本。 |
...
| `autoBootstrap` | `true` | 空链自举：会话首个 pre-step 在链表为空时把开场情景作为首节点提交。 |
| `selfCheckEnabled` | `true` | 回合末自检：完成的"有工作"回合在链头够老时提交状态节点。 |
| `selfCheckMinHeadAgeMs` | `300000`（5 分钟） | 回合末自检提交替换节点前要求的链头年龄下限。 |
| `selfCheckMinToolCalls` | `1` | 判定回合"确有工作"的工具调用下限（0 也计入纯聊天回合）。 |
| `compactionWriteBack` | `true` | 收到 `compaction/summary` 事件时，以摘要文本提交一个链节点（`origin: compaction`），让被挤出弧段的线索存活在基质中。 |
| `contextDepth` | `4` | 自举开场情景提取所取的尾部文本块数。 |
...
`situational_state_trace` 查询轨迹账本（`trace.jsonl`）。可按 `session_id`、`node_id`、`kind`（`inject`/`commit`）过滤，或用 `limit` 限制条数（默认 20，最大 100）；结果最新在前，含 `traceId`、`seq`、`nodeId`、`kind`、`sessionId`、`situation`、`createdAt` 与 `position`（事件发生的会话 `seq:`）。commit 条目还携带 `origin`（`tool`/`bootstrap`/`turn-end`/`compaction`），标明节点由哪条路径创建。
...
每个提交节点每个 agent 注入一次（由 per-agent 最近注入映射守卫）。链表为空且 `autoBootstrap` 关闭时零 token；开启自举时，链在首个 pre-step 获得开场节点。
...
- **唤醒是提醒而非强制提交** — agent 可忽略检查点；除自动自举与回合末自检外，不再次调用 `situational_state_commit` 就不会写状态。
