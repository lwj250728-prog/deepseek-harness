...
在不定义工具的情况下添加模型可见的请求上下文的产品插件。`agent-instructions` 包含在默认 `dsh-agent-spine-demo` 组合包中，可通过组合包配置禁用；`time-context`、`tmux-context`、`session-reference` 和 `cognitive-inject` 需主动启用。
...
| [`cognitive-inject/`](cognitive-inject/README.md) | 步骤级 SAR 经验预热 | — |
| [`situational-state/`](situational-state/README.md) | 自决情景状态链表 | `ctx.situationalState` |
