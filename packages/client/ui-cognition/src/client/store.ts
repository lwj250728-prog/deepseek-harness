...
import type {
  LearningActions, LearningFilter, LearningState,
  LifeActions, LifeState,
...
/** Empty life-strip snapshot before the first fetch. */
...
/**
 * Create the digital-life strip's viewing store: the last fetched overview
 * (chain head + trace tail + designation) plus the body-expansion state.
 * @returns the store handle.
 */
export function createLifeStore(): EngineStoreHandle<LifeState, LifeActions> {
  return defineStore({
    init: (): LifeState => ({ ...INITIAL_LIFE_STATE }),
    actions: {
      replace(state, snapshot) {
        state.status = 'ready'
        state.head = snapshot.head
        state.trace = snapshot.trace
        state.designatedSessionId = snapshot.designatedSessionId
        state.error = null
      },
      begin(state) {
        state.status = 'loading'
