/** `cognition` namespace dictionaries. */

/** Dictionary namespace owned by this plugin. */
export const NS = 'cognition'

/** Simplified Chinese dictionary (the key-set source of truth). */
export const zh = {
  'area.label': '学习会话',
  'area.label.expanded': '学习会话（{count}）',
  'count.running': '{count} 个学习中',
  'count.total': '{count} 个任务',
  'status.pending': '待执行',
  'status.running': '学习中',
  'status.completed': '已完成',
  'status.failed': '已失败',
  'filter.all': '全部',
  'filter.pending': '待执行',
  'filter.running': '学习中',
  'filter.completed': '已完成',
  'filter.failed': '已失败',
  'action.refresh': '刷新',
  'empty': '暂无学习任务',
  'empty.filtered': '没有符合筛选条件的任务',
  'goal.label': '目标',
  'result.label': '结果',
  'createdAt.label': '创建于',
  'list.aria': '学习任务列表',
  'refresh.aria': '刷新学习任务',
  'error.load': '加载学习任务失败',
  'bubble.title': '本回合认知沉淀',
  'bubble.experiences': '新经验 {count} 条',
  'bubble.citations': '引用 {cited}/{settled} 条注入',
  'bubble.resolved': '结算 {count} 次',
  'bubble.expand.aria': '展开认知沉淀明细',
  'bubble.collapse.aria': '收起认知沉淀明细',
  'bubble.topic.prefix': '主题：',
  'bubble.empty.detail': '无明细',
  'life.label': '生命',
  'life.designated': '主对话',
  'life.empty': '生命状态链尚未开始；首个有实质工作的回合会自动建立。',
  'life.age': '{age}',
  'life.owner': '会话 {session}',
  'life.kind.inject': '注入',
  'life.kind.commit': '提交',
  'life.origin.tool': '工具',
  'life.origin.bootstrap': '自举',
  'life.origin.turn-end': '回合末',
  'life.origin.compaction': '压缩',
  'life.refresh.aria': '刷新生命状态',
  'life.toggle.aria': '展开生命条',
} as const

/** English dictionary, key-identical to the Chinese source of truth. */
export const en: Record<CognitionKey, string> = {
  'area.label': 'Learning',
  'area.label.expanded': 'Learning ({count})',
  'count.running': '{count} learning',
  'count.total': '{count} tasks',
  'status.pending': 'pending',
  'status.running': 'learning',
  'status.completed': 'completed',
  'status.failed': 'failed',
  'filter.all': 'All',
  'filter.pending': 'Pending',
  'filter.running': 'Learning',
  'filter.completed': 'Completed',
  'filter.failed': 'Failed',
  'action.refresh': 'Refresh',
  'empty': 'No learning tasks yet',
  'empty.filtered': 'No tasks match the filter',
  'goal.label': 'Goal',
  'result.label': 'Result',
  'createdAt.label': 'Created',
  'list.aria': 'Learning task list',
  'refresh.aria': 'Refresh learning tasks',
  'error.load': 'Failed to load learning tasks',
  'bubble.title': 'This turn\'s cognition',
  'bubble.experiences': '{count} new experience(s)',
  'bubble.citations': '{cited}/{settled} injections cited',
  'bubble.resolved': '{count} prediction(s) resolved',
  'bubble.expand.aria': 'Expand cognition detail',
  'bubble.collapse.aria': 'Collapse cognition detail',
  'bubble.topic.prefix': 'Topic: ',
  'bubble.empty.detail': 'No detail',
  'life.label': 'Life',
  'life.designated': 'main',
  'life.empty': 'The life state chain has not started yet; the first substantive turn establishes it.',
  'life.age': '{age}',
  'life.owner': 'session {session}',
  'life.kind.inject': 'recall',
  'life.kind.commit': 'commit',
  'life.origin.tool': 'tool',
  'life.origin.bootstrap': 'bootstrap',
  'life.origin.turn-end': 'turn-end',
  'life.origin.compaction': 'compaction',
  'life.refresh.aria': 'Refresh life state',
  'life.toggle.aria': 'Expand life strip',
}

/** Key domain of the `cognition` namespace (zh is the source of truth). */
export type CognitionKey = keyof typeof zh
