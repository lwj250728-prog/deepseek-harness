...
import type {
  ExplorationTaskStatus, ExplorationTaskView,
  LifeChainHead, LifeTraceEntry,
} from '@deepseek-ai/dsh-api-remotes/client'
import type { createLearningStore, createLifeStore } from '../store.ts'
...
// ── digital-life strip (sidebar.life) ───────────────────────────────────────

/** Life-strip snapshot store state. */
export interface LifeState {
  /** Fetch lifecycle: idle until the first refresh. */
  status: 'idle' | 'loading' | 'ready' | 'error'
  /** The current chain head, null before the chain starts. */
  head: LifeChainHead | null
  /** The newest trace entries, newest first. */
  trace: readonly LifeTraceEntry[]
  /** The session designated as the main conversation (null before any). */
  designatedSessionId: string | null
  /** Load/refresh failure message, null while healthy. */
  error: string | null
  /** Whether the strip body is expanded. */
  expanded: boolean
}

/** Life-strip store actions. */
export type LifeActions = {
  /** Apply a fetched overview snapshot; clears any stale error. */
  replace: (state: LifeState, snapshot: {
    head: LifeChainHead | null
    trace: readonly LifeTraceEntry[]
    designatedSessionId: string | null
  }) => void
  /** Mark a fetch in flight. */
  begin: (state: LifeState) => void
  /** Record a fetch failure. */
  fail: (state: LifeState, message: string) => void
  setExpanded: (state: LifeState, expanded: boolean) => void
}

/** The inject face: the only place the strip touches the wire. */
export interface LifeInjected {
  /** Fetch the life overview; resolves when the snapshot replaces the store. */
  refresh: (signal: AbortSignal) => Promise<void>
}

/** Full life-strip component props (owner state + store + inject + locale). */
export type LifeStripProps =
  PropsRuntime<'sidebar.life'>
  & PropsStore<ReturnType<typeof createLifeStore>>
  & LifeInjected
  & PropsLocale<'cognition'>
