...
flowchart LR
  ...
  pkg_cognition_cognitive_pipeline["cognition/cognitive-pipeline"]
  svc_cognitivePipeline["ctx.cognitivePipeline<br/>Prediction-error-driven cognition pipeline"]
  pkg_e2b["e2b"]
  ...
  pkg_cognition_cognitive_pipeline --> svc_cognitivePipeline
...
| `ctx.cognitivePipeline` | `core` | `cognition/cognitive-pipeline` | - | - | - | 持有 SAR 经验存储与热/冷双环路引擎：带 OOD 路由的校准预测、反馈修正、以及沙盒门控的效用空间分类法重建。 |
